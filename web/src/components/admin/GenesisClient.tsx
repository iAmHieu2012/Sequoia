...
/**
 * Genesis Page (Admin Dashboard Core).
 * Provides a comprehensive, cyberpunk-themed interface for managing Nebulas (Topics),
 * Stars (Articles), AI Models, and Textbooks. Integrates the Cosmos Map Editor for visual mapping.
 */
export default function GenesisClient() {
  ...
  const handleSave = async (payload: Record<string, unknown>) => {
    try {
      if (forgeTab === 'stars' || forgeTab === 'anomalies') await AdminService.saveArticle(payload);
      else if (forgeTab === 'nebulas') await AdminService.saveTopic(payload);
      else if (forgeTab === 'models') await AdminService.saveModel(payload);
  ...
  const handleDelete = async (tab: AdminTab | "stars", item: { id: string } & Partial<Topic & Article & AiModel & Textbook>, e: React.MouseEvent) => {
    ...
    try {
      if (tab === 'stars' || tab === 'anomalies') await AdminService.deleteArticle(item.id);
      else if (tab === 'nebulas') await AdminService.deleteTopic(item.id);
