...
export default function CosmosMapEditor({ targetX, targetY, targetScale = 0.2, mapId, activeNodeId, className = "", refreshKey }: CosmosMapEditorProps) {
    ...
    const onMouseMove = (e: MouseEvent) => {
      if (draggingNodeId) {
        setLocalNodes(nodes => nodes.map(n => 
          n.articleId === draggingNodeId 
            ? { ...n, x: n.x + e.movementX / currentScaleRef.current, y: n.y + e.movementY / currentScaleRef.current }
            : n
        ));
      }
    };
  ...
  useEffect(() => {
    if (activeNodeId && localNodes.length > 0) {
      const activeNode = localNodes.find(n => n.articleId === activeNodeId);
      if (activeNode) {
  ...
  const handleNodeMouseDown = (e: React.MouseEvent, nodeId: string) => {
    ...
    // Yêu cầu zoom 1x trước khi tương tác
    if (Math.abs(currentScaleRef.current - 1.0) > 0.01) {
      const targetNode = localNodes.find(n => n.articleId === nodeId);
      if (targetNode) {
        flyTo(targetNode.x, targetNode.y, 1.0);
      }
      return;
    }
    ...
    <div className={`relative bg-black/60 border border-panel-border overflow-hidden ${className}`}>
      ...
      <div className="absolute top-3 left-3 z-20 pointer-events-none flex flex-col gap-2">
        ...
        <div className="bg-black/80 border border-white/20 p-2 text-[9px] font-mono text-white/60">
          ...
          {linkingNodeId && <div className="text-orange mt-1 animate-pulse">SELECT TARGET...</div>}
        </div>
      </div>
          ...
          <svg className={`${styles.lightBeams} absolute inset-0 w-full h-full overflow-visible z-[2]`}>
            {localNodes.flatMap(node =>
              node.connections.map((connId: string) => {
                const target = localNodes.find(n => n.articleId === connId);
            ...
            {linkingNodeId && (
              <line 
                x1={localNodes.find(n => n.articleId === linkingNodeId)?.x || 0} 
                y1={localNodes.find(n => n.articleId === linkingNodeId)?.y || 0} 
              ...
              return (
                ...
                >
                  {draggingNodeId === node.articleId && (
                    <div className="absolute -top-6 left-1/2 -translate-x-1/2 bg-black/80 border border-cyan/50 text-cyan px-2 py-0.5 text-[8px] font-mono whitespace-nowrap z-50 pointer-events-none">
                  ...
                  {isAnomaly ? (
                    ...
                    <>
                      ...
                      <div className={`${styles.objectLabel} flex flex-col items-center gap-1 ${isCompleted ? 'text-cyan drop-shadow-[0_0_10px_var(--color-cyan)]' : ''}`}>
